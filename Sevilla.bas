...
apikey = "80e7a4533c268c7a859032cb5a805a2a"  ' -->registro sev
...
'apikey = "4c5e5c7f3a329c623c721cb64f3ca110" 'nuria "80e7a4533c268c7a859032cb5a805a2a"   ' -->registro sev
    ...
    If Items.Count > 0 Then
        If InputBox("Se procederá a realizar el envío de Notificaciones DE PAPEL y PUBLICIDAD, series 1 y 5. Introduzca la palabra PAPEL para confirmar") = "PAPEL" Then
           ...
           MsgBox "Envío finalizado. Revise la bandeja de salida y la carpeta ConError."
        Else
...
INICIO: If Items.Count > 0 Then
        ...
        Else
            If InputBox("Se procederá a realizar el envío de Notificaciones DE DEPOSITO DE CUENTAS, serie 2. Se enviarán dos correos al cliente, uno con los documentos adjuntos y otro con el enlace a GPFD." & Chr(13) & "Introduzca la palabra CUENTAS para confirmar") = "CUENTAS" Then
               ...
               '''''
               'Application.Session.SendAndReceive True 'ENVIAR Y RECIBIR
               '''''
...
LOPD10 = " y garantía de los derechos digitales (BOE núm.  294, de 6 de diciembre de 2018)."
    ...
    'If MergePDF3s Then
    If GeneraPDF3 Then
        EnviaDeposito = EnvioFacturas(apikey, factura, tipo, mail, asunto, body1, fecha, importe, "c:\GPFD\merged.pdf")
    Else
        CodError = "Se ha producido un error al procesar los ficheros adjuntos."
        GoTo ErrorEnvia
    End If
...
Function GuardaAdjuntos(mensaje As MailItem) As Boolean
...
'If TypeName(Application.ActiveWindow) = "Inspector" Then
    ...
    If mensaje.Attachments.Count > 0 Then
        i = mensaje.Attachments.Count
        For Each adjunto In mensaje.Attachments 'explora cada adjunto en el mensaje
            adjunto.SaveAsFile (carpeta & i & ".pdf") 'adjunto.FileName)
            i = i - 1
        Next
...
error_guardar:
GuardaAdjuntos = 0
MsgBox "Error en los adjuntos de: " & factura
...
body = Limpia_Espacios(Limpia_guiones(body))
    ...
    If GeneraPDF2 Then
        ...
        EnviaTelematica = EnvioFacturas(apikey, factura, tipo, mail, asunto, body1, fecha, importe, "c:\GPFD\merged.pdf") ' envio a gpfd
    Else
        CodError = "Se ha producido un error al procesar los ficheros adjuntos."
        GoTo ErrorEnvia
    End If
...
Private Function GeneraPDF3() As Boolean
'
On Error GoTo err
Dim cadenaPDFs As String
Dim miArchivo1, miArchivo2, miMerge As String

miArchivo1 = Dir("c:\gpfd\1.pdf")
miArchivo2 = Dir("c:\gpfd\2.pdf")
miMerge = Dir("c:\gpfd\merged.pdf")

    If miMerge = "merged.pdf" Then Kill ("c:\gpfd\merged.pdf")
    
    If miArchivo1 = "1.pdf" And miArchivo2 = "2.pdf" Then
        cadenaPDFs = "c:\gpfd\Aviso1.pdf;c:\gpfd\1.pdf;c:\gpfd\2.pdf"
        cadena = "C:\Program Files (x86)\NAPS2\naps2.console -i " & cadenaPDFs & " -n 0 -o c:\gpfd\merged.pdf -f" '& numerodoc & ".pdf
        x = Shell(cadena, vbNormalFocus)
        Pause 3
        GeneraPDF3 = True
        Exit Function
    Else
        MsgBox "No se encuentra el fichero adjunto"
    End If

err:
MsgBox "Error en ficheros adjuntos"
GeneraPDF3 = False
End Function

Private Function GeneraPDF2() As Boolean
'
On Error GoTo err
Dim cadenaPDFs As String
Dim miArchivo1, miMerge As String
...
miMerge = Dir("c:\gpfd\merged.pdf")
    ...
    If miArchivo1 = "1.pdf" Then
        cadenaPDFs = "c:\gpfd\Aviso1.pdf;c:\gpfd\1.pdf"
        cadena = "C:\Program Files (x86)\NAPS2\naps2.console -i " & cadenaPDFs & " -n 0 -o c:\gpfd\merged.pdf -f" '& numerodoc & ".pdf
        x = Shell(cadena, vbNormalFocus)
        Pause 3
        GeneraPDF2 = True
        Exit Function
    Else
        MsgBox "No se encuentran los ficheros adjuntos"
    End If
...
Set myCopiedItem = Item.Copy 'copio el mail
myCopiedItem.Move olFolder.Folders("GPFD") 'muevo la copia a GPFD

Item.body = nuevobody 'le cambio el body
    

Set oMail = Item 'lo selecciono
For Each oAccount In Application.Session.Accounts
    If oAccount.AccountType = olPop3 Then
        'oMail.Recipients.ResolveAll
        oMail.SendUsingAccount = oAccount
         oMail.Send 'LO ENVIA A LA BANDEJA DE SALIDA
    End If
Next
...
'sandbox
'http = "http://pruebasgpfd.sitelcom.es/api/documento_nuevo.php"

'produccion actual
'http = "http://gpfd.es/api/test.php"
http = "https://gpfd.es/api/documento_nuevo.php"
...
'Create the multipart form data
    ...
    If web.readyState = 4 Then
        ...
        ElseIf web.Status = 408 Then 'Debug never run to here?
